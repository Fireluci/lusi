        ...
        if len(message.text) < 100:
            ...
            for x in find:
                if x in removes:
                    ...
                    search = search + x + " "
            search = re.sub(r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|new|dub|dubb|netflix|series|latest|bruh|broh|helo|dubbed|link|venum|iruka|pannunga|pannungga|anuppunga|anupunga|anuppungga|anupungga|film|undo|kitti|kitty|tharu|kittumo|kittum|movie|any(one)|full\smovie)|with\ssubtitle(s)?)", "", search, flags=re.IGNORECASE)
...
async def advantage_spell_chok(client, msg):
    ...
    for x in find:
        if x in removes:
            ...
            query = query + x + " "
    query = re.sub(r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|gib)(\sme)?)|full\smovie)|web\sseries|netflix|film|dubb|dub(bed)|movie(s)?|with\ssubtitle(s)?)", "", query, flags=re.IGNORECASE)
